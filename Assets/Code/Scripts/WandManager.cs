...
namespace Code.Scripts
{
    ...
    public class Wand : MonoBehaviour, IGameboardCanvasPointer
    {
        ...
        private List<Vector3> ComputeArc(float gravity, Vector3 initialVelocity, float yPlane)
        {
            var wandManager = WandManager.Instance;
            ...
            var impact = ComputeImpactPoint(points, yPlane);
            if (impact.HasValue) points[^1] = impact.Value; 
            
            return points;
        }
        
        private static Vector3? ComputeImpactPoint(List<Vector3> points, float yPlane)
        {
            if (points.Count < 2)
            {
                return null;
            ...
            var t = (yPlane - pointBelow.y) / (pointAbove.y - pointBelow.y);
        ...
        private void DrawArc()
        {
            ...
            _points = ComputeArc(gravity, velocity, WandManager.Instance.yPlane);
            ...
            var impactPoint = ComputeImpactPoint(_points, WandManager.Instance.yPlane);
